...
function startDebugging(manifestPath, options) {
    var _a, _b, _c;
    return __awaiter(this, void 0, void 0, function* () {
        const { appType, app, debuggingMethod, sourceBundleUrlComponents, devServerCommandLine, devServerPort, packagerCommandLine, packagerHost, packagerPort, enableDebugging, enableLiveReload, enableSideload, openDevTools, document, local, sourceBundleOverrideFile, } = Object.assign(Object.assign({}, options), { 
        ...
        try {
            ...
            if (isDesktopAppType && isWindowsPlatform) {
                if (local) {
                    const diskManifestDir = (0, shared_1.getDiskManifestDir)(true /* create */);
                    const diskManifestFile = path_1.default.join(diskManifestDir, path_1.default.basename(manifestPath));
                    yield fs_1.default.copyFileSync(manifestPath, diskManifestFile);
                    const enableDiskManifests = yield devSettings.enableDiskManifests(diskManifestDir);
                    console.log("Using local disk manifest : " + enableDiskManifests);
                }
                if (sourceBundleOverrideFile) {
                    const sourceBundleOverrideFilePath = path_1.default.resolve(sourceBundleOverrideFile);
                    yield devSettings.enableSourceBundleOverrideFile(manifestInfo.id, sourceBundleOverrideFilePath);
            ...
            if (enableSideload) {
                try {
                    ...
                    console.log(msg);
                    yield (0, office_addin_dev_settings_1.sideloadAddIn)(manifestPath, app, true, launchOnly, appType, document);
                }
                catch (err) {
